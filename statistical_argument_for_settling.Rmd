...
Given $max(X,n)$ has CDF $F(x)^n$, the PDF is given by $ d/dx CDF = n * F(X)^(n-1) * f(x)$. Therefore, to find the expectation we evaluate the following interval:
...
Aaaaand we're already at an impasse. This integral doesn't just *look* difficult to evaluate: according to wolfram.com, even if we adopt the normality assumption, closed form solutions are only known up to $n=5$: http://mathworld.wolfram.com/GumbelDistribution.html  
...
Looks like an ok fit, but it seems to be shifted up a bit for some reason. I'm not sure exactly how to account for that, but I'm guessing there's some kind of bias introduced by my methodology.
