...
$p_n=F(F^{-1}_{X_(n)}(.5))$
$T_n=\frac{1}{p}=\frac{1}{F(F^{-1}_{X_(n)}(.5))}$
...
CI = apply(test, 2, function(x) sapply(q, function(qi) xv[which.min(qi>x)]))
plot(0, 0, type='n', xlim=c(min(N), max(N)), ylim=c(min(CI), ylim=max(CI)))
lines(N, CI[1,], col='red', lty=2)
lines(N, CI[2,])
lines(N, CI[3,], col='red', lty=2)
```
