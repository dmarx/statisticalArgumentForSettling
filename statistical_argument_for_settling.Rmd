...
```{r}
plot(test$values, log(test$next_arrival_wait), col=(1+floor(log10(test$arrival_time))))
...
# 2^n isn't a bad estimate, but it appears to actually grow *faster* than this. Let's improve
# on this estimate later.
xv = 1:20
...
I don't really have experience with extreme value theory, so I'm probably doing this the hard way. Here we go!

Given max(X,n) has CDF F(x)^n, PDF = d/dx CDF = n * F(X)^(n-1) * f(x)

$$
\begin{align}
  E[x|n] &= \int x pdf dx
  E[x|n] &= \int x *  n * F(X)^(n-1) * f(x)
\end{align}
$$

Aaaaand it looks like we're at an impasse here. I tried plugging this into Wolfram Alphaand no beans, and unfortunately I don't think I'm talented enough with calculus to turn this into something meaningful, even with the additional constrant of the normality assumption.

In fact, this integral doesn't just *look* difficult: according to wolfram.com, even if we adopt the normality assumption, closed form solutions are only known up to $n=5$. 

Although an analytic solution isn't readily available, we can still use numerical methods to approximate this integral at different values of n to get a numerical estimate for this expectation, and then maybe fit a curve to the result to get an approximation to the analytic solution as a function of n. Although it'll just be an approximation, it may still be very informative of the behavior of this expectation. In particular, I'm interested to see how quickly this grows, since 2^n fails for large n.
...
## Method 2. True monte carlo simulation of mean
...
sapply(1:4,function(x) lines(density(max_sim_given_n(10^x, 5e3)), col=x))
...
# The observed convergence in log space is presumably to the standard gumbel distribution
...
Method 2 should be completely sufficient, but just for fun since we have that integral, it'll be interesting to numerically approximate the integral itself. Frankly, I think this will actually produce better estimates than either of the above solutions (which is the main reason I'm doing this) but we won't know for sure until after experimentation (because I'm too lazy to  get out pen and paper and estimate the magnitude of my estimated error for these different approaches).

Although this method will probably give us more accurate values for the expectation, we will *not* have confidence bounds for the statistic anymore, which, as we learned above, is extremely important to understanding the behavior of this statistic when $n$ is large.

...Honestly, I may just skip this method. Or fuck it, I'll just do it cause why not. 

```{r}
# I could just use the built in numerical integration function, but what's the fun in that?
mc_int = function(fx, xrange=c(0,1), ymax=NA, iters=1e6, trace=FALSE){
  xmin = min(xrange); xmax = max(xrange); ymin = 0
  if(is.na(ymax)){
